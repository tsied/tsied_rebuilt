...
(function($) {
	...
	$.fn.datetimepicker = function(opt) {
		var KEY0 = 48,
			...
			lazyInitTimer = 0,
			createDateTimePicker, destroyDateTimePicker,
...
/*scrollbar = $('<div class="xdsoft_scrollbar"></div>'),
				...
				XDSoft_datetime,
				//scroll_element,
				xchangeTimer, timerclick, current_time_index, setPos, timer = 0,
				timer1 = 0,
				_xdsoft_datetime;
				...
				if (_options.highlightedDates && $.isArray(_options.highlightedDates) && _options.highlightedDates.length) {
					$.each(_options.highlightedDates, function(index, value) {
						var splitData = $.map(value.split(','), $.trim),
							exDesc, hDate = new HighlightedDate(Date.parseDate(splitData[0], options.formatDate), splitData[1], splitData[2]),
							// date, desc, style
				...
				if (_options.highlightedPeriods && $.isArray(_options.highlightedPeriods) && _options.highlightedPeriods.length) {
					...
					$.each(_options.highlightedPeriods, function(index, value) {
						var splitData = $.map(value.split(','), $.trim),
							...
							desc = splitData[2],
							hDate, keyDate, exDesc, style = splitData[3];
			...
			XDSoft_datetime = function() {
				...
				_this.nextMonth = function() {
					...
					Math.min(
					new Date(_this.currentTime.getFullYear(), month + 1, 0).getDate(), _this.currentTime.getDate()));
