...
(function(e) {
	...
	e.extend(m.prototype, {
		...
		text: function(a, b) {
			...
			c.conf.text = b;
			d ? c.node.children(d).html(h(b)) : c.node.html(h(b));
			return this
		},
		toIndex: function(a) {
			var b, c, d, e;
		...
		_buildButtons: function(a, b, c) {
			...
			for (var h = 0, i = a.length; h < i; h++) {
				...
				if (k) if (e.isArray(k)) this._buildButtons(k, b, c);
				else {
					var g = this._buildButton(k, c !== n ? !0 : !1);
					if (g) {
						var r = g.node;
						b.append(g.inserter);
		...
		_keypress: function(a, b) {
			...
			var i = this.s.subButtons,
				k = function(c, d) {
					if (c.key) if (c.key === a) d.click();
					else if (e.isPlainObject(c.key) && c.key.key === a && (!c.key.shiftKey || b.shiftKey)) if (!c.key.altKey || b.altKey) if (!c.key.ctrlKey || b.ctrlKey)(!c.key.metaKey || b.metaKey) && d.click()
				};
			c = 0;
	...
	m.instanceSelector = function(a, b) {
		...
		var c = [],
			...
			f = function(a) {
				if (e.isArray(a)) for (var i = 0, k = a.length; i < k; i++) f(a[i]);
	...
	j.Api.register("buttons.exportData()", function(a) {
		if (this.context.length) {
			for (var b = new j.Api(this.context[0]), c = e.extend(!0, {}, {
				...
				}).toArray() : null, h = b.rows(c.rows, c.modifier).indexes().toArray(), h = b.cells(h, c.columns).render(c.orthogonal).toArray(), i = a.length, k = 0 < i ? h.length / i : 0, g = Array(k), l = 0, m = 0; m < k; m++) {
